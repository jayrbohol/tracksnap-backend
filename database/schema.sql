-- TrackSnap Database Schema (PostgreSQL/Supabase)
-- Created: October 2025
-- Description: Complete database schema for TrackSnap parcel tracking system
...
-- Spatial indexes for location-based queries (using standard PostgreSQL)
-- Note: For advanced geospatial features, consider enabling PostGIS extension
...
-- Function to update geocoding cache usage
CREATE OR REPLACE FUNCTION update_geocoding_cache_usage()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE geocoding_cache 
    SET last_used = NOW(), use_count = use_count + 1 
    WHERE address_hash = NEW.address_hash;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
-- Create admin user functions (if using Row Level Security)
-- These would be customized based on your authentication system

-- =====================================================
-- OPTIONAL: ADVANCED GEOSPATIAL FEATURES (PostGIS)
-- =====================================================
-- Uncomment the following section if you want to enable PostGIS for advanced spatial operations
-- This requires installing the PostGIS extension: CREATE EXTENSION IF NOT EXISTS "postgis";

/*
-- Enable PostGIS extension (requires superuser privileges)
CREATE EXTENSION IF NOT EXISTS "postgis";

-- Add geometry columns for better spatial performance
ALTER TABLE parcels ADD COLUMN recipient_geom GEOMETRY(POINT, 4326);
ALTER TABLE parcels ADD COLUMN pickup_geom GEOMETRY(POINT, 4326);
ALTER TABLE parcels ADD COLUMN sortation_geom GEOMETRY(POINT, 4326);
ALTER TABLE parcels ADD COLUMN delivery_hub_geom GEOMETRY(POINT, 4326);
ALTER TABLE tracking_events ADD COLUMN geom GEOMETRY(POINT, 4326);
ALTER TABLE couriers ADD COLUMN last_known_geom GEOMETRY(POINT, 4326);
ALTER TABLE hubs ADD COLUMN geom GEOMETRY(POINT, 4326);

-- Create spatial indexes (much faster for location queries)
CREATE INDEX idx_parcels_recipient_geom ON parcels USING gist(recipient_geom);
CREATE INDEX idx_parcels_pickup_geom ON parcels USING gist(pickup_geom);
CREATE INDEX idx_parcels_sortation_geom ON parcels USING gist(sortation_geom);
CREATE INDEX idx_parcels_delivery_hub_geom ON parcels USING gist(delivery_hub_geom);
CREATE INDEX idx_tracking_events_geom ON tracking_events USING gist(geom);
CREATE INDEX idx_couriers_geom ON couriers USING gist(last_known_geom);
CREATE INDEX idx_hubs_geom ON hubs USING gist(geom);

-- Function to update geometry columns from lat/lng
CREATE OR REPLACE FUNCTION update_geometry_columns()
RETURNS TRIGGER AS $$
BEGIN
    -- Update parcel geometries
    IF TG_TABLE_NAME = 'parcels' THEN
        IF NEW.recipient_lat IS NOT NULL AND NEW.recipient_lng IS NOT NULL THEN
            NEW.recipient_geom := ST_SetSRID(ST_MakePoint(NEW.recipient_lng, NEW.recipient_lat), 4326);
        END IF;
        ...
        END IF;
        IF NEW.sortation_lat IS NOT NULL AND NEW.sortation_lng IS NOT NULL THEN
            NEW.sortation_geom := ST_SetSRID(ST_MakePoint(NEW.sortation_lng, NEW.sortation_lat), 4326);
        END IF;
        IF NEW.delivery_hub_lat IS NOT NULL AND NEW.delivery_hub_lng IS NOT NULL THEN
            NEW.delivery_hub_geom := ST_SetSRID(ST_MakePoint(NEW.delivery_hub_lng, NEW.delivery_hub_lat), 4326);
        END IF;
    END IF;
    
    -- Update tracking event geometry
    IF TG_TABLE_NAME = 'tracking_events' THEN
        NEW.geom := ST_SetSRID(ST_MakePoint(NEW.lng, NEW.lat), 4326);
    END IF;
    
    -- Update courier geometry
    IF TG_TABLE_NAME = 'couriers' THEN
        IF NEW.last_known_lat IS NOT NULL AND NEW.last_known_lng IS NOT NULL THEN
            NEW.last_known_geom := ST_SetSRID(ST_MakePoint(NEW.last_known_lng, NEW.last_known_lat), 4326);
        END IF;
    END IF;
...
-- Apply geometry update triggers
CREATE TRIGGER update_parcels_geometry BEFORE INSERT OR UPDATE ON parcels
    FOR EACH ROW EXECUTE FUNCTION update_geometry_columns();
CREATE TRIGGER update_tracking_geometry BEFORE INSERT OR UPDATE ON tracking_events
    FOR EACH ROW EXECUTE FUNCTION update_geometry_columns();
CREATE TRIGGER update_couriers_geometry BEFORE INSERT OR UPDATE ON couriers
    FOR EACH ROW EXECUTE FUNCTION update_geometry_columns();
CREATE TRIGGER update_hubs_geometry BEFORE INSERT OR UPDATE ON hubs
    FOR EACH ROW EXECUTE FUNCTION update_geometry_columns();

-- Enhanced distance function using PostGIS (more accurate than Haversine)
CREATE OR REPLACE FUNCTION postgis_distance_meters(lat1 DECIMAL, lng1 DECIMAL, lat2 DECIMAL, lng2 DECIMAL)
RETURNS INTEGER AS $$
BEGIN
    RETURN ST_Distance(
...
-- Example spatial queries you can use with PostGIS:
-- Find parcels within 5km of a point:
-- SELECT * FROM parcels WHERE ST_DWithin(recipient_geom, ST_SetSRID(ST_MakePoint(121.0244, 14.5547), 4326)::geography, 5000);

-- Find nearest hub to a parcel:
-- SELECT h.* FROM hubs h ORDER BY ST_Distance(h.geom, ST_SetSRID(ST_MakePoint(121.0244, 14.5547), 4326)::geography) LIMIT 1;
*/

COMMENT ON DATABASE tracksnap IS 'TrackSnap Parcel Tracking System Database';
